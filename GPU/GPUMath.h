...
// ---------------------------------------------------------------------------------------
/*
__device__ __noinline__ void _ModInvNoGrouped(uint64_t r[4]) {
...
// ---------------------------------------------------------------------------------------
/*
__device__ __noinline__ void _ModInvNoGrouped2(uint64_t r[2][4]) {
  ...
  Load256(subp[0], r[0]);
  _ModMult(subp[1], subp[0], r[1]);

  // We need 320bit signed int for ModInv
  Load256(inverse, subp[1]);
  inverse[4] = 0;
  _ModInv(inverse);
    
  
  _ModMult(newValue, subp[0], inverse);
  _ModMult(inverse, r[1]);
  Load256(r[1], newValue);
