...
void CheckAddress(Secp256K1 *T, std::string address, std::string privKeyStr) {
  ...
  Point pub = T->ComputePubKey(&privKey);
...
void Secp256K1::SelfTest() {
  ...
  Point pub = ComputePubKey(&privKey);
...
Point Secp256K1::ComputePubKey(Int *privKey) {
  ...
  for(; i < 32; i++) {
    b = privKey->GetByte(i);
    if(b)
      Q = AddJacobian(Q, GTable[256 * i + (b-1)]);
  }
...
Point Secp256K1::ParsePublicKeyHex(std::string str, bool &isCompressed) {
  ...
  if (str.length() < 2) {
    printf("\n[FATAL_ERROR] invalid public key (len must be 66 or 130)\n");
	exit(EXIT_FAILURE);
  }
  ...
  switch (type) {
    ...
    case 0x02:
      if (str.length() != 66) {
        printf("\n[FATAL_ERROR] invalid public key (len must be 66)\n");
		exit(EXIT_FAILURE);
      }
      for (int i = 0; i < 32; i++)
    ...
    case 0x04:
      if (str.length() != 130) {
        printf("\n[FATAL_ERROR] invalid public key (len must be 130)\n");
		exit(EXIT_FAILURE);
      }
      for (int i = 0; i < 32; i++)
        ret.x.SetByte(31 - i, GetByte(str, i + 1));
    ...
    default:
      printf("\n[FATAL_ERROR] invalid public key (prefix must be 02,03,04)\n");
	  exit(EXIT_FAILURE);
  }
...
// J + J -> J ( 12M, 2S) (secp256k1 has 12M, 4S)
Point Secp256K1::AddJacobian(Point &p1, Point &p2) {
  ...
  */
  ...
  u1.ModMulK1(&p2.y, &p1.z); // 1M
  u2.ModMulK1(&p1.y, &p2.z); // 1M
  v1.ModMulK1(&p2.x, &p1.z); // 1M
  v2.ModMulK1(&p1.x, &p2.z); // 1M
  u.ModSub(&u1, &u2);
  v.ModSub(&v1, &v2);
  w.ModMulK1(&p1.z, &p2.z); // 1M
  us2.ModSquareK1(&u); // 1S
  vs2.ModSquareK1(&v); // 1S
  vs3.ModMulK1(&vs2, &v); // 1M
  us2w.ModMulK1(&us2, &w); // 1M
  vs2v2.ModMulK1(&vs2, &v2); // 1M
  _2vs2v2.ModAdd(&vs2v2, &vs2v2);
  a.ModSub(&us2w, &vs3);
  a.ModSub(&_2vs2v2);
  ...
  vs3u2.ModMulK1(&vs3, &u2); // 1M
  r.y.ModSub(&vs2v2, &a);
  r.y.ModMulK1(&r.y, &u); // 1M
  r.y.ModSub(&vs3u2);

  r.z.ModMulK1(&vs3, &w); // 1M
...
Point Secp256K1::DoubleAffine(Point &p) {
  ...
  // s = (3*pow2(p.x)) * inverse(p.y + p.y);
  // rx = pow2(s)  - (p.x + p.x);
  // ry = neg(p.y + s*(ret.x - p.x));

  Int _s, _p, _q;
  Point r;
  r.z.SetInt32(1);

  _s.ModSquareK1(&p.x); // 1S
  _p = _s;
  _p.ModDouble();
  _p.ModAdd(&_s);
  ...
  _q.ModDouble();
  _q.ModInv(); // 1I
  _s.ModMulK1(&_p, &_q); // 1M

  _p.ModSquareK1(&_s); // 1S
  _q = p.x;
  _q.ModDouble();
  r.x.ModSub(&_p, &_q);    
...
// 2J -> J (7M, 5S) (secp256k1 has 4M, 4S)
Point Secp256K1::DoubleJacobian(Point &p) {
  ...
  */
  ...
  z2.ModSquareK1(&p.z); // 1S
  z2.SetInt32(0); // a=0
  x2.ModSquareK1(&p.x); // 1S
  _3x2.ModAdd(&x2, &x2);
  _3x2.ModAdd(&x2);
  w.ModAdd(&z2, &_3x2);
  s.ModMulK1(&p.y, &p.z); // 1M
  b.ModMulK1(&p.y, &s); // 1M
  b.ModMulK1(&p.x); // 1M
  h.ModSquareK1(&w); // 1S
  _8b.ModAdd(&b, &b);
  ...
  s2.ModSquareK1(&s); // 1S
  y2.ModSquareK1(&p.y); // 1S
  _8y2s2.ModMulK1(&y2, &s2); // 1M
  _8y2s2.ModDouble();
  _8y2s2.ModDouble();
  _8y2s2.ModDouble();
...
// 2 * A -> A (1I, 2M, 2S)
Point Secp256K1::Mult2Affine(Point &p) {

	//s = p.x^2 * invert(p.y + p.y) % mod;			# 1I, 1M, 1S
	//s = (s + s + s) % mod;
	//r.x = (s^2 - p.x - p.x) % mod;				# 1S
	//r.y = (s*(p.x - r.x) - p.y) % mod;			# 1M
	//return r
...
// kA -> A
Point Secp256K1::MultKAffine(Int &k, Point &p) {
	...
	//			return Ap
	//	elif(k % 2 == 0) :
	//			return mul_ka(k/2, mul_2a(Ap))
	//	else :
	//			return add_a(Ap, mul_ka((k-1)/2, mul_2a(Ap)))
	...
	if (k.IsZero()) {
		...
		k2 = k;
		k2.ShiftR(1);
		//return MultKAffine(k2, Mult2Affine(p));
		//add
		Point dp = DoubleAffine(p);
		return MultKAffine(k2, dp);
		//return MultKAffine(k2, DoubleAffine(p));
	} else {
		k2 = k;
		k2.SubOne();
		k2.ShiftR(1);
		//return AddAffine(p, MultKAffine(k2, Mult2Affine(p)));
		//add
		Point dp2 = DoubleAffine(p);
		Point k2dp = MultKAffine(k2, dp2);
		return AddAffine(p, k2dp);
		//return AddAffine(p, MultKAffine(k2, DoubleAffine(p)));
	}
